...
struct irq_key{
    ...
    wait_queue_head_t rwait;  //读等待队列头
...
ssize_t dev_read (struct file * filp, char __user * buf, size_t count, loff_t * ppos)
{
    ...
    if(filp->f_flags&O_NONBLOCK)    //以非阻塞方式打开
    {
        
    }
    else                            //以阻塞方式打开
    {
        wait_event_interruptible(pkey0->rwait, pkey0->read_state);       //等待按键值有效
...
void key1_timer_func(unsigned long arg)
{
    ...
    wake_up_interruptible(&irq_key0->rwait);
    ...
    irq_key0->read_state = READ_VALID;
}
...
static int __init alientek_dev_init(void)
{
    ...
    //初始化等待队列头
    init_waitqueue_head(&dev.key[0].rwait);
