...
int key_dev_open (struct inode *inode, struct file *filp)
{
    filp->private_data = &dev;
    return 0;
}
int key_dev_close (struct inode *inode, struct file *filp)
{
    return 0;
}
ssize_t key_dev_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
    return count;
}
...
static void timer_handler(unsigned long arg)
{
    ...
    input_report_key(dev->input,INPUT_CODE,KEY_PRESS);      //上报按键值
    input_sync();
    input_event(dev->input,EV_REP,INPUT_CODE,KEY_PRESS);    //上报按下次数
    input_sync();
...
{
 	mod_timer(&((struct dev*)work)->timer, jiffies + msecs_to_jiffies(10));
}
