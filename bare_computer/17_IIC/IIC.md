...
| 特性               | 同步通信                     | 异步通信                     |
|--------------------|----------------------------|----------------------------|
| **时钟信号**       | 有独立的时钟线(SCL/SCK)     | 无时钟线                   |
| **数据同步方式**   | 依赖时钟边沿采样            | 依赖预定义的波特率和起始位 |
| **硬件复杂度**     | 需要时钟同步电路            | 只需数据线                 |
| **典型协议**       | SPI, I2C                   | UART                       |
...
- ALS（Ambient Light Sensor）：测量周围环境的光照强度，单位通常为勒克斯（Lux）。
- PS（Proximity Sensor）：检测物体与传感器的接近程度，通常用于判断是否有物体靠近。通常为一个 16 位无符号整数，表示反射光强度。
数值越大，表明物体越近；但非线性关系，需通过校准转换为距离。
- IR（Infrared Sensor）：测量环境中的红外光强度，可用于检测特定物体或环境特性。通常为一个 16 位无符号整数，表示红外光强度。
