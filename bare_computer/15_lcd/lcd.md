...
```mermaid
graph LR
    A[main.c] -->|#include point.h| B[main.o]
    C[utils.c] -->|#include point.h| D[utils.o]
    E[app.c] -->|#include point.h| F[app.o]
    ...
    D --> G
    F --> G
    G --> K["类型定义被丢弃<br>仅保留实际使用的代码"]
    end
```

|特性|	类型定义(struct/union/enum)|	变量/函数定义|
|----|----|----|
|是否分配内存|	❌ 否|	✅ 是|
|是否产生符号|	❌ 否|	✅ 是|
|是否受ODR约束|	宽松（允许多副本）|	严格（仅允许一份）|
|条件编译作用|	防单个文件内重复定义|	**无法防跨文件多重定义**|
|链接器处理|	忽略类型定义|	检查符号唯一性|
